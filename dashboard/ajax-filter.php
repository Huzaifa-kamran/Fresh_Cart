...
`categoryID` WHERE `proStatus` = '1' AND `inStock` = '1'";

}elseif($filter == 0){

    $products = "SELECT * FROM `products` INNER JOIN `categories` ON `products`.`catID` = `categories`.
`categoryID` WHERE `proStatus` = '0'";

}elseif($filter == 2){

    $products = "SELECT * FROM `products` INNER JOIN `categories` ON `products`.`catID` = `categories`.
    `categoryID` WHERE `proStatus` = '1' AND `inStock` = '0'";
}else{
    $products = "SELECT * FROM `products` INNER JOIN `categories` ON `products`.`catID` = `categories`.
    `categoryID`";
}
...
$output= "";
 ...
 if(mysqli_num_rows($res)>0){
  ...
  $output .=  '</td>
    ...
    $output .= "<h5 class='ms-5 ps-4'>No Products Found</h5>";
    echo $output;
 }
